...
app.all('/result4/', (req, res) => {
    ...
    if (x_test && req.rawBody) {
        ...
        res.json({message: "iusupova_anastasiia", "x-result": x_test, "x-body": req.rawBody});
    } else {
        res.setHeader("Content-Type", "application/json");
        res.header("Access-Control-Allow-Headers", "x-text, x-test");
        res.json({message: "iusupova_anastasiia", "x-result": x_test});
    }
    res.json({message: "iusupova_anastasiia"});
